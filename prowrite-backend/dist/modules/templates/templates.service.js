...
let TemplatesService = class TemplatesService {
    ...
    async hasProPlusSubscription(workspaceId) {
        const { data: subscriptionData } = await this.supabaseService.subscriptions
            .select('*')
            .eq('workspace_id', workspaceId)
            .eq('status', database_1.SubscriptionStatus.ACTIVE)
            .single();
        const subscription = subscriptionData;
        if (!subscription) {
            return false;
        }
        return subscription.plan_type === database_1.PlanType.PRO || subscription.plan_type === database_1.PlanType.ENTERPRISE;
    }
    async getTemplatesByModule(moduleType, workspaceId) {
        ...
        const filtered = (templates || []).filter((t) => t.workspace_id === null || t.workspace_id === workspaceId);
        console.log('Filtered templates count:', filtered.length);
        return filtered;
    }
    async getTemplate(id, workspaceId) {
    ...
    async createTemplate(dto, workspaceId) {
        const hasProPlus = await this.hasProPlusSubscription(workspaceId);
        if (!hasProPlus) {
            throw new common_1.ForbiddenException('Custom templates require a Pro or Enterprise subscription');
        }
    ...
    getColdEmailTemplates() {
        return [
            {
                module_type: database_1.ModuleType.COLD_EMAIL,
...
Output only the message text, no formatting.`,
    ...
    getWebsiteCopyTemplates() {
        return [
            {
                module_type: database_1.ModuleType.WEBSITE_COPY,
                name: 'Landing Page',
...
META_DESCRIPTION: [SEO meta description]`,
    ...
    getHRDocsTemplates() {
        return [
            {
                module_type: database_1.ModuleType.HR_DOCS,
...
SUMMARY: [overall summary and recommendations]`,
    ...
    getYouTubeScriptsTemplates() {
        return [
            {
                module_type: database_1.ModuleType.YOUTUBE_SCRIPTS,
